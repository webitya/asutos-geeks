        ...
        {step === 1 ? (
          <form onSubmit={handleCredentialsCheck} className="space-y-6">
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              New here? <Link href="/register" className="text-primary font-bold">Create an account</Link>
            </p>
          </form>
        ) : (
          <form onSubmit={handleVerifyLogin} className="space-y-6">
            <p className="text-center text-gray-600 text-sm">Verify your identity with the code sent to <b>{formData.email}</b></p>
            <input
              type="text"
              required
              maxLength={6}
              placeholder="000000"
              className="w-full text-center text-3xl tracking-[1.5rem] bg-gray-50 border border-gray-200 rounded-xl px-5 py-5 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
              value={otp}
